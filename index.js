...
app.post("/batch-watermark", async (req, res) => {
  try {
    ...
    // ✅ track total page usage across files
    let totalPages = 0;
    ...
    const results = await Promise.all(
      files.map(async ({ user_email, file, lender }, i) => {
        try {
          ...
          if (usageRecord.page_credits - usageRecord.pages_used < totalPages + numPages) {
    ...
    // ✅ update usage after all files processed
    if (usageRecord && totalPages > 0) {
      await supabase
        .from("usage")
        .update({
          pages_used: usageRecord.pages_used + totalPages,
          pages_remaining: usageRecord.page_credits - usageRecord.pages_used - totalPages,
        })
        .eq("user_email", usageRecord.user_email);
    }
